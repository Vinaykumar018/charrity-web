...
import {
  MdArticle, MdPerson, MdCalendarToday, MdLocationOn, MdTitle,
  MdSave, MdDelete, MdAdd, MdSearch, MdImage, MdDescription,
  MdEdit, MdVisibility, MdClose, MdArrowBack, MdCheckCircle
} from "react-icons/md";
import { FaCloudUploadAlt, FaImage, FaNewspaper } from "react-icons/fa";
...
export default function AdminBlogs() {
  ...
  function validateForm() {
    ...
    if (!form.dateDisplay) {
      newErrors.dateDisplay = "Please select a date";
    }
    if (!form.day || !form.month) {
      newErrors.dateDisplay = "Invalid date selected";
    }
  ...
  async function loadPage(p = 1, q = search) {
    try {
      setLoading(true);
      const res = await api.getBlogs(p, perPage, q);
      console.log("API Response:", res);

      if (Array.isArray(res)) {
        setBlogs(res);
        setTotalBlogs(res.length);
        setTotalPages(Math.ceil(res.length / perPage));

        if (q) {
          const filtered = res.filter(blog =>
  ...
  async function saveBlog() {
    ...
    try {
      ...
      const payload = { ...form };
      delete payload._id;
      await api.updateBlog(selectedId, payload);
  ...
  async function removeBlog() {
    ...
    try {
      setSaving(true);
      await api.deleteBlog(blogToDelete);
      ...
      toast.error(e.message || "Failed to delete blog. Please try again.");
  ...
  // Delete Confirmation Modal (defined inside the component)
    ...
    <AdminLayout>
      ...
      <div className="b-root p-6">
        <div style={{ maxWidth: 1400, margin: "0 auto" }}>
          {/* Header */}
          <div className="b-page-title">Blog Management</div>
          <div className="b-page-sub">Create, edit, and manage your blog posts</div>
          <div className="b-accent-bar" />

          {/* Stats Cards */}
          <div className="b-stats-grid">
            <div className="b-stat-card">
              <div className="b-stat-icon"><FaNewspaper /></div>
            ...
            <div className="b-stat-card">
              <div className="b-stat-icon"><MdPerson /></div>
              <div className="b-stat-content">
                <h3>{new Set(blogs.map(b => b.author)).size}</h3>
                <p>Authors</p>
              </div>
            </div>
            <div className="b-stat-card">
          ...
          <div className="b-header-actions">
            ...
            <button className="b-btn-primary" onClick={handleNewBlog}>
              <MdAdd size={18} />
              Create New Blog
            </button>
          </div>
          ...
          {loading ? (
            ...
            <div className="b-empty-state">
              <FaNewspaper className="b-empty-state-icon" />
              <h3>No blogs found</h3>
              <p>Get started by creating your first blog post</p>
              <button className="b-btn-primary" onClick={handleNewBlog} style={{ display: "inline-flex" }}>
                <MdAdd size={18} />
                Create First Blog
              </button>
            </div>
            ...
            <>
              <div className="b-blogs-grid">
                {getCurrentPageBlogs().map((blog) => (
                  <div key={blog._id} className="b-blog-card">
                    ...
                    <div className="b-blog-content">
                      <h3 className="b-blog-title">{blog.title || "Untitled"}</h3>
                      <div className="b-blog-meta">
                        <span><MdPerson /> {blog.author || "Unknown"}</span>
                        {blog.location && (
                          <span><MdLocationOn /> {blog.location}</span>
                        )}
                      </div>
                      <p className="b-blog-excerpt">{blog.excerpt || "No excerpt available"}</p>
              ...
              {totalPages > 1 && (
                <div className="b-pagination">
                  <span className="b-pagination-info">
                    Showing page {page} of {totalPages} · {totalBlogs} total blogs
                  </span>
          ...
          {showForm && (
            <div className="b-form-overlay">
              <div className="b-form-modal">
                <div className="b-form-header">
                  <h2>
                    {selectedId ? <><MdEdit /> Edit Blog</> : <><MdAdd /> Create New Blog</>}
                  </h2>
                ...
                <div className="b-form-body">
                  ...
                  <div className="b-grid-2">
                    ...
                    <div className="b-field">
                      <label className="b-label">
                        <MdPerson className="b-label-icon" />
                        Author <span className="b-badge-req">Required</span>
                      </label>
                      <input
                        className={`b-input${errors.author ? " err" : ""}`}
                        value={form.author || ""}
                        onChange={(e) => {
                      ...
                      {errors.author && (
                        <div className="b-err-msg">
                          <MdVisibility size={12} /> {errors.author}
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="b-grid-4" style={{ marginTop: "16px" }}>
                    <div className="b-field">
                      ...
                      <input
                        type="date"
                        className={`b-input${errors.dateDisplay ? " err" : ""}`}
                  ...
                  {errors.dateDisplay && (
                    <div className="b-err-msg" style={{ marginTop: "8px" }}>
                      <MdVisibility size={12} /> {errors.dateDisplay}
                    </div>
                  )}
                  ...
                  <div className="b-field" style={{ marginTop: "20px" }}>
                    <label className="b-label">
                      ...
                      Excerpt <span className="b-badge-req">Required</span>
                    </label>
                    <textarea
                      className={`b-textarea${errors.excerpt ? " err" : ""}`}
                      rows={3}
                      value={form.excerpt || ""}
                      onChange={(e) => {
                    ...
                    {errors.excerpt && (
                      <div className="b-err-msg">
                        <MdVisibility size={12} /> {errors.excerpt}
                      </div>
                    )}
                  </div>
                  ...
                  <div className="b-field" style={{ marginTop: "20px" }}>
                    <label className="b-label">
                      <MdDescription className="b-label-icon" />
                      Content <span className="b-badge-req">Required</span>
                    </label>
                    <textarea
                      className={`b-textarea${errors.content ? " err" : ""}`}
                      rows={8}
                      value={form.content || ""}
                      onChange={(e) => {
                    ...
                    {errors.content && (
                      <div className="b-err-msg">
                        <MdVisibility size={12} /> {errors.content}
                      </div>
                    )}
                  </div>
                </div>
